...
def run_stellenbosch_tests():
    ...
    mode = input("Weighted (w), Uniform (u), or Spatial Decay (s)? ").strip().lower()
    NUM_RUNS = 5
    ...
    print("\n" + "="*60)
    print(f"STELLENBOSCH TEST ({NUM_RUNS} runs with averaging)")
    print("="*60)
    
    print("\n\\begin{tabular}{llllll}")
    print("\\textbf{Type} & \\textbf{Nodes} & \\textbf{p} & \\textbf{Weights} & \\textbf{Demand Cov} & \\textbf{Graph Cov} \\\\ \\hline")
    ...
    for ratio in [0.001, 0.005]:
        ...
        for run in range(NUM_RUNS):
            ...
            random.seed(seed)
            if mode == 's':
                weights = generate_spatial_demand(G, poi_nodes, w_min=5, w_max=25, decay_rate=2.5)
                weight_type = "Spatial decay"
            elif mode == 'w':
